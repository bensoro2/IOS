...
const translations: Record<string, Record<string, string>> = {
  ...
  "sub.checkoutError": { th: "เกิดข้อผิดพลาดในการสร้าง checkout", en: "Error creating checkout", ja: "チェックアウト作成エラー", zh: "创建结账时出错", ko: "결제 생성 오류", ru: "Ошибка создания оплаты" },
   ...
   "terms.s3.body": { th: "ผู้ใช้งานตกลงที่จะไม่โพสต์หรือส่งเนื้อหาที่ผิดกฎหมาย เป็นภัยคุกคาม หรือหลอกลวง ผู้งานต้องรับผิดชอบต่อความปลอดภัยของตนเองในการเข้าร่วมกิจกรรมต่างๆที่เกิดขึ้นผ่านการนัดหมายในแอปพลิเคชัน", en: "Users agree not to post or share illegal, threatening, or fraudulent content. Users are responsible for their own safety when participating in activities arranged through the application.", ja: "ユーザーは、違法、脅迫的、または詐欺的なコンテンツを投稿・共有しないことに同意します。アプリを通じて調整されたアクティビティに参加する際の安全はユーザー自身が責任を負います。", zh: "用户同意不发布或分享违法、威胁性或欺诈性内容。通过本应用安排参与活动时，用户需自行承担安全责任。", ko: "사용자는 불법, 위협적, 또는 사기성 콘텐츠를 게시하거나 공유하지 않기로 동의합니다. 애플리케이션을 통해 약속된 활동 참여 시 안전은 사용자 본인이 책임집니다.", ru: "Пользователи соглашаются не публиковать незаконный, угрожающий или мошеннический контент. Ответственность за безопасность при участии в мероприятиях несёт сам пользователь." },
   ...
   "terms.s5.body": { th: "Level, EXP หรือไอเทมต่างๆ ในระบบถือเป็นกรรมสิทธิ์ของแอปพลิเคชัน และไม่มีมูลค่าเป็นเงินจริง ไม่สามารถแลกเปลี่ยนเป็นเงินสดได้", en: "Level, EXP, and other in-app items are the property of the application and have no real monetary value. They cannot be exchanged for cash.", ja: "レベル、EXP、その他のアイテムはアプリの所有物であり、実際の金銭的価値はありません。現金との交換はできません。", zh: "等级、EXP及系统内其他物品均为应用程序所有，不具有实际货币价值，不可兑换为现金。", ko: "레벨, EXP 및 기타 인앱 아이템은 애플리케이션의 재산이며 실제 금전적 가치가 없습니다. 현금으로 교환할 수 없습니다.", ru: "Уровень, EXP и другие внутриигровые предметы являются собственностью приложения и не имеют реальной денежной ценности. Обмен на наличные невозможен." },
   ...
   "terms.s6.body": { th: "เราขอสงวนสิทธิ์ในการระงับหรือยกเลิกบัญชีของผู้ใช้งานที่ละเมิดข้อกำหนดนี้โดยไม่ต้องแจ้งให้ทราบล่วงหน้า", en: "We reserve the right to suspend or terminate accounts that violate these terms without prior notice.", ja: "当社は、本規約に違反するアカウントを事前の通知なく停止または終了する権利を留保します。", zh: "我们保留暂停或终止违反本条款账户的权利，无需提前通知。", ko: "당사는 이 약관을 위반한 계정을 사전 통지 없이 정지 또는 종료할 권리를 보유합니다.", ru: "Мы оставляем за собой право приостанавливать или удалять аккаунты, нарушающие условия, без предварительного уведомления." },
   "terms.s7.title": { th: "7. การแก้ไขข้อตกลง", en: "7. Amendment of Terms", ja: "7. 規約の改定", zh: "7. 条款修改", ko: "7. 약관 수정", ru: "7. Изменение условий" },
   "terms.s7.body": { th: "เราอาจมีการปรับปรุงเปลี่ยนแปลงข้อตกลงนี้ได้ตลอดเวลาโดยจะประกาศให้ทราบผ่านทางแอปพลิเคชัน", en: "We may update these terms at any time. Changes will be announced through the application.", ja: "当社は随時本規約を改定することがあります。変更はアプリを通じてお知らせします。", zh: "我们可能随时更新这些条款，变更将通过应用程序公告。", ko: "당사는 수시로 약관을 업데이트할 수 있습니다. 변경사항은 애플리케이션을 통해 공지됩니다.", ru: "Мы можем обновлять эти условия в любое время. Изменения будут объявлены в приложении." },
